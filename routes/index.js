...
router.get("/comment", function (req, res) {
  res.render("comment", { footer: false });
});
router.get("/message", function (req, res) {
  res.render("message", { footer: false });
});
router.get("/notification", function (req, res) {
  res.render("notification", { footer: false });
});
...
router.post("/comment/:postid", async function (req, res) {
  try {
    ...
    const newComment = {
      user: user._id, // Assuming comment schema has a field for user ID
...
router.post("/register", function (req, res) {
  ...
  if (!req.body.email || !req.body.email.endsWith("@sggs.ac.in")) {
    return res.status(400).send(`
    <script>
      alert('Only email IDs ending with @sggs.ac.in are allowed.');
      window.location.href = '/'; // Redirect back to registration page
    </script>
  `);
  }
...
router.post(
  ...
  function(req, res, next) {
    passport.authenticate("local", function(err, user, info) {
      ...
      if (user.aporve) { // Assuming 'aporve' is a boolean variable
        req.logIn(user, function(err) {
          if (err) { return next(err); }
